...
function App() {
  ...
  const { isLogged, currentTransfer, client_id, detailedSingleTrans, device, themeColor, transferMoneyMobile } = useSelector<AppState, CompoProps>(
    (state) => ({
  ...
  const SubmitTransferModal = currentTransfer === TRANS_ERR || currentTransfer === TRANS_LOADING ? '' : <ConfirmTransfer />;
  return (
    <div className={styles.App} data-theme={themeColor}>
      {/* component to show transaction details */}
      {/* show it in mobile version only */}
      {device === Devices.DESKTOP ? detailedSingleTrans ? <DetailedSingleTransaction /> : '' : detailedSingleTrans ? <DetailedTrxMobile /> : ''}
      {/* submit transfer modal */}
