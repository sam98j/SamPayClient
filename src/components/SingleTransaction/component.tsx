...
import { hideDetailedSingleTrans, showDetailedSingleTrans } from '../../apis/system';
...
const SingleTransaction: React.FC<SingleTransProps> = (props) => {
  ...
  const detailedSingleTrans = useSelector<AppState, DetailedSingleTrans>(({ system }) => {
  ...
  return (
    <div className={styles.singleTransaction} onMouseEnter={handleMouseOver} onMouseLeave={handleMouseLeave}>
      <div className={styles.img}>{Boolean(receiver.avatar) ? <img src={receiver.avatar} /> : <p>{receiver.name[0]}</p>}</div>
